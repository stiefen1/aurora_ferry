...
class TrackedTarget:
    ...
    def update_from_ais(self, measurement: np.ndarray) -> None:
        ...
        self.vessel.north, self.vessel.east, self.vessel.sog, self.vessel.cog = self.tracker.x.tolist()
...
class NavigationAurora(INavigation):
    ...
    def __get__(self, eta:np.ndarray, nu:np.ndarray, current:Current, wind:Wind, obstacles:List[Obstacle], target_vessels:List[IVessel], timestamps: datetime,  *args, tau_actuators: Optional[np.ndarray] = None, **kwargs) -> Tuple[Dict, Dict]:
        """
        ...
        ais = self.sensors['ais'](timestamps=timestamps, max_age_seconds=self.max_age_seconds)
        ...
        for vessel in vessels_ais:
            ...
            if vessel.mmsi in self.target_collection:
                ...
                measurement = np.array([vessel.north, vessel.east, vessel.sog, vessel.cog])
            ...
            # New vessel -> Create tracked target
                ...
                tracker = TargetTrackerSequentialEKF(
                    **self.target_tracker_params, 
                    x0=np.array([vessel.north, vessel.east, vessel.sog, vessel.cog])
                )
